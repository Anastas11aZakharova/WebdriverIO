...
describe('https://www.saucedemo.com/', () => {
    ...
    it('Check footer links', async () => {
        ...
        await expect(ProductPage.twitterLink).toHaveAttribute("href", "https://twitter.com/saucelabs")
        await expect(ProductPage.facebookLink).toBeExisting()
        await expect(ProductPage.facebookLink).toHaveAttribute("href", "https://www.facebook.com/saucelabs")
        await expect(ProductPage.linkedinLink).toBeExisting()
        await expect(ProductPage.linkedinLink).toHaveAttribute("href", "https://www.linkedin.com/company/sauce-labs/")
    ...
    it('Valid checkout', async () => {
        ...
        await expect(CheckoutPage.yourInformationTitile).toHaveText("Checkout: Your Information")
        await CheckoutPage.enterCheckoutInformation(Helper.generateRandomString(10), Helper.generateRandomString(10), Helper.generateRandomString(4))
        ...
        await expect(CompletePage.completeMessage).toHaveText("Thank you for your order!")
    })
    ...
    it('Missing first name on checkout', async () => {
        ...
        await expect(CheckoutPage.yourInformationTitile).toBeExisting()
        await expect(CheckoutPage.yourInformationTitile).toHaveText("Checkout: Your Information")
        await CheckoutPage.enterCheckoutInformation("", Helper.generateRandomString(10), Helper.generateRandomString(4))
    ...
    it('Missing post code on checkout', async () => {
        ...
        await expect(CheckoutPage.yourInformationTitile).toBeExisting()
        await expect(CheckoutPage.yourInformationTitile).toHaveText("Checkout: Your Information")
        await CheckoutPage.enterCheckoutInformation(Helper.generateRandomString(10), Helper.generateRandomString(10), "")
        await expect(CheckoutPage.errorMessage).toBeExisting()
        await expect(CheckoutPage.errorMessage).toHaveText('Error: Postal Code is required')
        await ProductPage.clickOnBurgerBtn()
        await ProductPage.clickOnResetAppStateBtn()
    });
    it('Backpack price on product equals price on cart page', async () => {
        await LoginPage.open()
    ...
    it('Sorting', async () => {
        ...
        await expect(await ProductPage.getNamesSortingOrder()).toEqual('descending')
        await ProductPage.sortLohi()
        await expect(await ProductPage.getPricesSortingOrder()).toEqual('ascending')
        await ProductPage.sortHilo()
        await expect(await ProductPage.getPricesSortingOrder()).toEqual('descending')       
    })
    it('Backpack description on product equals discription on cart page', async () => {
        ...
        await expect(ProductPage.productsTitle).toBeExisting()
        await expect(ProductPage.backpackPrice).toBeExisting()
        var descriptionOnProductPage = await ProductPage.backpackDescription.getText()
        await ProductPage.clickOnBurgerBtn()
        await ProductPage.clickOnResetAppStateBtn()
        await ProductPage.clickOnAddToCartBtn()
        await expect(ProductPage.cartIcon).toBeExisting()
    ...
    it('Problem user sees only dog pictures', async () => {
        ...
        for await (const element of ProductPage.inventoryItemImage) {
            await expect(element).toHaveAttribute("src", "/static/media/sl-404.168b1cce.jpg")
        }
    ...
    it('The tax is added correctly during checkout', async () => {
        ...
        await expect(CartPage.yourCartTitle).toHaveText("Your Cart")
        await expect(CartPage.sauceLabsBackpackProduct).toBeExisting()
        var priceOnCartPage =parseFloat((await CartPage.backpackPrice.getText()).slice(1))
        await CartPage.clickOnCheckoutBtn()
        await expect(CheckoutPage.yourInformationTitile).toBeExisting()
        await expect(CheckoutPage.yourInformationTitile).toHaveText("Checkout: Your Information")
        await CheckoutPage.enterCheckoutInformation(Helper.generateRandomString(10), Helper.generateRandomString(10), Helper.generateRandomString(4))
        ...
        var totalPrice =parseFloat((await OverviewPage.totalPriceLabel.getText()).slice(8))
